...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public List<CustomerModel> getAllRecords(){
        ...
        if(cursor.moveToFirst())
        {
            do{
                ...
                CustomerModel newCustomerModel=new CustomerModel(customerName,custAge,isActive,customerId);
    ...
    public void deleteRecord(int row){
        SQLiteDatabase DB =this.getReadableDatabase();
        DB.delete(CUST_TABLE,"CustomerID=?",new String[] {String.valueOf(row)});
        DB.close();
    }
}
